...
const liturgyTypes = [
  {
    ...
    values: [
      ...
      {
        title:
          "ORDER FOR GRADUATION INTO YOUNG PEOPLE'S GUILD (YPG)/YOUNG ADULTS (YAF)/MEN/WOMEN'S FELLOWSHIPS ",
        content: ""
      },
      {
...
const LiturgyTypeSelection = ({ navigation }) => {
  ...
  function renderType(liturgyType) {
    return (
      <TouchableOpacity
        onPress={() =>
          navigateToLiturgyType(liturgyType.item.name, liturgyType.item.values)
        }
      >
          ...
          <View style={[{ flexDirection: "row" }]}>
            ...
            >
              <StyledTextInverse style={{ fontSize: 20 }}>
                {liturgyType.item.letter}
              </StyledTextInverse>
            </View>
            <View style={{ justifyContent: "center" }}>
              <StyledText style={{ fontSize: 20 }}>
                {liturgyType.item.name}
              </StyledText>
            </View>
          </View>
  ...
  return (
    <View style={[styles.container]}>
      <ChildScreenHeader title="Liturgy Book Selection" />

      <View style={[styles.list]}>
        <FlatList data={liturgyTypes} renderItem={renderType} />
      </View>
    </View>
  );
};
